...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query(value = "SELECT e.id AS id, " +
            ...
            "FROM public.user_event AS ue " +
            "INNER JOIN public.event AS e ON e.id = ue.event_id", nativeQuery = true)
     ...
     */
    @Modifying
    @Query(value = "UPDATE public.user_event SET is_sent = true WHERE user_email = ?1 AND event_id = ?2", nativeQuery = true)
    void updateSent(String email, Long id);
}
